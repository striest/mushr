...
class nik():
    ...
    def __init__(self, name, z_pos = 0.3089):
        ...
        self.motorHandles = []
        self.motorHandles.append(sim.simGetObjectHandle('joint_front_left_wheel'))
        self.motorHandles.append(sim.simGetObjectHandle('joint_front_right_wheel'))
        self.motorHandles.append(sim.simGetObjectHandle('joint_back_left_wheel'))
        self.motorHandles.append(sim.simGetObjectHandle('joint_back_right_wheel'))
    ...
    def get_joint_count(self) -> int:
        # nik is viewed as a 2 wheel model
        return 2
    ...
    def get_closest_n(self, obj_lis, n) -> List:
        if n>len(obj_lis):
            n=len(obj_lis)

        dis_lis = []
        for obj in obj_lis:
            twod_pos = obj.get_position(self)[:2]
            dis = np.sqrt((twod_pos[0]) ** 2 + (twod_pos[1]) ** 2)
            dis_lis.append(dis)

        ind = list(map(dis_lis.index, heapq.nsmallest(n, dis_lis)))
        result = []
        for obj in ind:
            result.append(obj_lis[obj])
        return result
    ...
    def set_velocity(self,lf,rf,lb,rb) -> None:
        ...
        for i in range(2):
            sim.simSetJointForce(self.motorHandles[2*i+1],self.joint_force)
            sim.simSetJointTargetVelocity(self.motorHandles[2*i+1],-v[2*i+1])

    def set_control_loop_enabled(self,value) -> None:
        for i in range(4):
            sim.simSetObjectInt32Parameter(
                self.motorHandles[i], sim.sim_jointintparam_ctrl_enabled, value)

    def set_motor_locked_at_zero_velocity(self,value) -> None:
        for i in range(4):
            sim.simSetObjectInt32Parameter(
                self.motorHandles[i], sim.sim_jointintparam_velocity_lock, value)
    ...
    def set_torque(self,lf,rf,lb,rb) -> None:
        ...
        for i in range(2):
            if t[2*i]>0:
                ...
                sim.simSetJointTargetVelocity(self.motorHandles[2*i],1000000000000000) #unreachable value
            else:
                sim.simSetJointForce(self.motorHandles[2*i],-t[2*i])
                sim.simSetJointTargetVelocity(self.motorHandles[2*i],-1000000000000000) #unreachable value
        for i in range(2):
            if t[2*i+1]>0:
                sim.simSetJointForce(self.motorHandles[2*i+1],t[2*i+1])
                sim.simSetJointTargetVelocity(self.motorHandles[2*i+1],-1000000000000000) #unreachable value
            else:
                sim.simSetJointForce(self.motorHandles[2*i+1],-t[2*i+1])
                sim.simSetJointTargetVelocity(self.motorHandles[2*i+1],1000000000000000) #unreachable value
    ...
    def check_distance(self, other: 'Object') -> float:
        ...
        return sim.simCheckDistance(
            self._handle, other.get_handle(), -1)[6]
