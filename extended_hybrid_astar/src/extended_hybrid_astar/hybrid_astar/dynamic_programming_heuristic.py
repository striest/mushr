...
#need to adjust later or will cause problems
xwidth = 200
ywidth = 200
resolution = 5
class Node:
...
def verify_node(node, obstacle_map, min_x, min_y, max_x, max_y):
    ...
    # collision check
    #I added this first if statement to deal with going out of bounds
    # if (math.floor(xwidth/resolution) <= int(node.x)) or (math.floor(ywidth/resolution) <= int(node.y)):
    #     #print(int(node.x))
    #     #print(int(node.y))
    #     return False

    # print(math.floor(xwidth/resolution))
    # print(node.x)
...
def calc_obstacle_map(ox, oy, resolution, vr):
    ...
    # zip in python 2 can be expensive, lets see if this helps by taking it out - ended up shaving off ~.05-.1 seconds so far :(
    # obstacle map generation
    obstacle_map = [[False for _ in range(y_width)] for _ in range(x_width)]
    for ix in range(x_width):
        x = ix + min_x
