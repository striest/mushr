...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        for i in range(obstacle_num):
            ...
            ob_pos = list(np.random.uniform(pos_min, pos_max))
            ob_or = [0, 0, np.random.uniform(-np.pi, np.pi)]
        ...
        starting_pose = [1.6,-1.6,starting_pose[2]]
        # new_pose = starting_pose
        # print(new_pose)
        agent.set_2d_pose(starting_pose)

        # Get a random position within a cuboid and set the target position
        target_pos = list(np.random.uniform(position_min, position_max))
